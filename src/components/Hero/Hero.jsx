...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    // device pixel ratio for crisp visuals on HiDPI displays
    ...
    // set canvas pixel size with DPR scaling
    const setSize = () => {
    ...
    // lightweight particle implementation
    class Particle {
      ...
      draw() {
        // soft radial gradient for each particle (subtle glow)
    ...
    // draw connections (using squared distance to avoid sqrt cost until needed)
    ...
    // debounced resize handler - rAF friendly
    const onResize = () => {
      if (resizeTimerRef.current) clearTimeout(resizeTimerRef.current);
      // small debounce to avoid thrash on mobile
      resizeTimerRef.current = setTimeout(() => {
        setSize();
        // re-init to adapt density nicely
        initParticles();
      }, 120);
    };
  ...
  return (
    <header className="hero" role="banner" aria-label="Hero — Alex M. Muli">
      ...
      {/* subtle glass layer to increase legibility */}
